...
from .utils cimport fscal, fcopy, fnrm2, fdot, faxpy
from .utils cimport LASSO, create_accel_pt
...
cpdef gsr_mtl(
        ...
        floating[:, :] W, floating[::1, :] R, floating[::1, :] theta,
        floating[:, :] XTR,
        floating[:] norms_X_col, uint8[:] screened, int max_iter,
        int gap_freq=10, float tol=1e-6, bint wstr_plus=False,
        bint verbose=0, bint use_accel=1, int K=6):
    if floating is double:
        dtype = np.float64
    else:
        dtype = np.float32
    ...
    cdef int i, j, k, epoch
    cdef int inc = 1
    cdef floating tmp
    cdef int n_obs = n_samples * n_tasks
    cdef floating p_obj, d_obj, scal, gap, radius
    cdef floating highest_d_obj = 0
    ...
    for epoch in range(max_iter):
        if epoch % gap_freq == 0:
            ...
            scal = 0.
            # do not use dual_scaling_mtl here, in order to maintain XTR:
            for j in range(n_features):
                if screened[j]:
                    continue
                for k in range(n_tasks):
                    XTR[j, k] = fdot(&n_samples, &R[0, k], &inc, &X[0, j], &inc)
                tmp = fnrm2(&n_tasks, &XTR[j, 0], &inc)
                if tmp > scal:
                    scal = tmp

            fcopy(&n_obs, &R[0, 0], &inc, &theta[0, 0], &inc)
            tmp = 1. / max(scal, alpha)
            fscal(&n_obs, &tmp, &theta[0, 0], &inc)
            d_obj = dual_mtl(n_samples, n_tasks, theta, Y, norm_Y2, alpha)
            ...
            if use_accel:
                ...
                if epoch // gap_freq >= K:
                    scal = dual_scaling_mtl(
                        n_features, n_samples, n_tasks, theta_acc, X,
                        n_features, &dummy_C[0], &screened[0], &Xj_theta[0])
                    if scal > 1.:
                        tmp = 1. / scal
                        fscal(&n_obs, &tmp, &theta_acc[0, 0], &inc)
            ...
            gap = p_obj - highest_d_obj
            if verbose:
                print("Iter %d: primal %.10f, gap %.2e" % (epoch, p_obj, gap),
                      end="")
            ...
            for j in range(n_features):
                ...
                if (tmp + radius * norms_X_col[j] < 1. and
                        fnrm2(&n_tasks, &W[j, 0], &inc) == 0.):
                    screened[j] = 1
                    n_screened += 1
            if verbose:
                print(" (%d feats left)" % (n_features - n_screened))
        ...
        for j in range(n_features):
            ...
            for k in range(n_tasks):
                XTR[j, k] = fdot(&n_samples, &X[0, j], &inc, &R[0, k], &inc)
                W[j, k] += XTR[j, k] / norms_X_col[j] ** 2
            BST(n_tasks, &W[j, 0], alpha / norms_X_col[j] ** 2)
